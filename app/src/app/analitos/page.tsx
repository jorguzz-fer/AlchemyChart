...
const EMPTY_FORM: FormState = {
  name: "", unit: "", level: "1", equipmentId: "", materialId: "",
};
...
export default function AnalitosPage() {
  const [items, setItems] = useState<Analyte[]>([]);
  const [equipments, setEquipments] = useState<Equipment[]>([]);
  const [materials, setMaterials] = useState<Material[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [editItem, setEditItem] = useState<Analyte | null>(null);
  ...
  const fetchAll = useCallback(async () => {
    ...
    setItems(a);
    setEquipments(e.filter((eq: Equipment & { active?: boolean }) => eq.active !== false));
    setMaterials(m.filter((mt: Material & { active?: boolean }) => mt.active !== false));
    setLoading(false);
  }, []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const url = editItem ? `/api/analitos/${editItem.id}` : "/api/analitos";
  ...
  const handleToggleActive = async (item: Analyte) => {
    await fetch(`/api/analitos/${item.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  const filtered = items.filter(
    (i) =>
      ...
      i.material.name.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            Gerencie os analitos vinculados a equipamentos e materiais
          </p>
        </div>
        <button
          onClick={openCreate}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-[#0c0b0b]">
                <tr>
                  {["Analito", "Unidade", "Nível", "Equipamento", "Material", "Status", "Ações"].map((h, i) => (
                    <th
                      key={h}
                      className={`px-6 py-3 text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider ${i === 6 ? "text-center" : "text-left"}`}
                    >
                      {h}
                    </th>
                  ))}
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-primary-50 text-primary-700">
                        {LEVEL_LABELS[item.level] ?? `Nível ${item.level}`}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{item.equipment.name}</td>
                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{item.material.name}</td>
                    <td className="px-6 py-4">
                      <span
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#141414] rounded-2xl border border-gray-100 dark:border-[#1a1a1a] p-6 w-full max-w-lg shadow-2xl mx-4">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="2">2 — Patológico</option>
              ...
              <div>
                ...
                {equipments.length === 0 && (
                  <p className="text-xs text-warning-600 mt-1">
                    Nenhum equipamento ativo. Cadastre equipamentos primeiro.
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {materials.length === 0 && (
                  <p className="text-xs text-warning-600 mt-1">
                    Nenhum material ativo. Cadastre materiais primeiro.
                  </p>
                )}
              </div>
