...
interface ImportResult {
  ...
  source: { extractedAt: string; controls: number; filePath: string };
}

export default function ImportQualichartRunsPage() {
  ...
  const handleRun = async () => {
    if (
      !confirm(
        "Importar todas as corridas históricas do QualiChart? Operação idempotente — não duplica corridas já existentes."
      )
    )
      return;
    setRunning(true);
    ...
    setResult(null);
    try {
      const res = await fetch("/api/admin/import-qualichart-runs", { method: "POST" });
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="text-xl font-bold text-black dark:text-white">
            Importar corridas QualiChart API
          </h1>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#141414] rounded-2xl border border-gray-100 dark:border-[#1a1a1a] p-5 space-y-3">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 text-sm text-gray-600 dark:text-gray-400 space-y-2">
            <p>
              Esta operação importa <strong>todas as corridas históricas</strong> de{" "}
              <strong>83 controles</strong> extraídas diretamente da API do QualiChart (~75 mil corridas).
            </p>
            <ul className="list-disc list-inside space-y-0.5 ml-2">
              <li>Equipamentos: HEMATO 01/02, AU 480 (1)/(2), Immulite 2000</li>
              <li>Materiais históricos criados automaticamente por lote</li>
              <li>Corridas excluídas no QualiChart são ignoradas</li>
            </ul>
            <p className="pt-1">
              <strong>Idempotente:</strong> pode rodar várias vezes. Corridas já existentes
              (mesmo vínculo + valor + dia) são ignoradas.
            </p>
            <p className="text-warning-600 dark:text-warning-400">
              <strong>Pré-requisito:</strong> executar <em>Importar dados QualiChart</em>{" "}
              antes para garantir que analitos e equipamentos existam. Pode demorar{" "}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
            <div className="bg-white rounded-lg p-3 border border-success-200">
              ...
              <div className="text-2xl font-bold text-success-700">
                {result.summary.runsCreated.toLocaleString("pt-BR")}
              </div>
            </div>
            <div className="bg-white rounded-lg p-3 border border-success-200">
              <div className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">
              ...
              <div className="text-2xl font-bold text-gray-500">
                {result.summary.runsSkipped.toLocaleString("pt-BR")}
              </div>
            </div>
            <div className="bg-white rounded-lg p-3 border border-success-200">
              <div className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">
                Excluídas (QualiChart)
              </div>
              <div className="text-2xl font-bold text-gray-500">
                {result.summary.deletedSkipped.toLocaleString("pt-BR")}
              </div>
            </div>
            <div className="bg-white rounded-lg p-3 border border-success-200">
              <div className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">
            ...
            <div className="bg-white rounded-lg p-3 border border-success-200">
              <div className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">
                Controles processados
              </div>
              <div className="text-2xl font-bold text-gray-500">
                {result.summary.controlsProcessed}
              </div>
            </div>
          </div>

          <div className="text-xs text-gray-500 pt-2 border-t border-success-200">
            Extraído em {new Date(result.source.extractedAt).toLocaleString("pt-BR")} —{" "}
            {result.source.controls} controles — arquivo:{" "}
            <code className="bg-gray-100 px-1 rounded">{result.source.filePath.split("/").pop()}</code>
          </div>

          {result.summary.skippedControls.length > 0 && (
            <div className="pt-2 border-t border-success-200">
              <div className="text-xs font-semibold text-warning-600 mb-1">
                {result.summary.skippedControls.length} controle(s) ignorado(s):
              </div>
              <ul className="text-xs text-gray-500 list-disc list-inside space-y-0.5">
                {result.summary.skippedControls.map((s, i) => (
                  <li key={i}>{s}</li>
                ))}
              </ul>
            </div>
